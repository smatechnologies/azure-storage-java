...
After extract, copy the Enterprise Manager Job-Subtype from the <root installation>/emplugins directory to dropins directory of each Enterprise Manager that will create job
...
Create a global property **AzureStoragePath** that contains the full path to the <root installation>.
...
-------------- | ---- | -----------
STORAGE | Text | <name> is the storage account name, <encrypted value> is the storage account connection string encrypted using the **EncryptValue.exe** program. 
